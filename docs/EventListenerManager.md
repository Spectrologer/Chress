...
this.eventManager.setupButton('buttonId', (e) => {
    console.log('Button clicked!');
}, { once: true });
```
...
// Custom options
this.eventManager.addKeyboardShortcut('Enter', (e) => {
    this.submitForm();
...
```typescript
this.eventManager.addPointerSequence(element, {
    onDown: (e) => {
...
```typescript
this.eventManager.addContextMenu(element, (e) => {
