/**
 * Chesse Service Worker - REFERENCE IMPLEMENTATION
 *
...
const CACHE_VERSION = 'chesse-v1.0.1';
            ...
            .then((cacheNames) => {
                return Promise.all(
                    cacheNames.map((cacheName) => {
                        if (cacheName.startsWith('chesse-') && cacheName !== CACHE_NAME &&
    ...
    const html = `
        ...
        <body>
            <div class="offline-container">
                ...
                <p>Chesse requires an internet connection to load new content.</p>
...
self.addEventListener('message', (event) => {
    ...
    if (event.data && event.data.type === 'CLEAR_CACHE') {
        caches.keys().then(cacheNames => {
            return Promise.all(
                cacheNames.map(cacheName => {
                    if (cacheName.startsWith('chesse-')) {
                        return caches.delete(cacheName);
                    }
                })
            );
