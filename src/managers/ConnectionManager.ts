...
export class ConnectionManager {
    public zoneConnections: Map<string, ZoneConnections>;

    constructor() {
        this.zoneConnections = new Map(); // Stores exit connections between zones
    }

    public getZoneKey(x: number, y: number): string {
        return `${x},${y}`;
    }

    public generateChunkConnections(centerX: number, centerY: number): void {
        ...
        for (let dx = -1; dx <= 1; dx++) {
            for (let dy = -1; dy <= 1; dy++) {
                ...
                const connections: ZoneConnections = {
    ...
    public getDeterministicExit(side: Direction, zoneX: number, zoneY: number): number | null {
    ...
    public ensureMinimumConnectivity(zoneX: number, zoneY: number, connections: ZoneConnections): void {
        ...
        const dist = Math.max(Math.abs(zoneX), Math.abs(zoneY));
        const zoneLevel = getZoneLevelFromDistance(dist);
    ...
    public ensureAdjacentConnections(zoneX: number, zoneY: number, connections: ZoneConnections): void {
    ...
    public getConnections(zoneX: number, zoneY: number): ZoneConnections | undefined {
        return this.zoneConnections.get(this.getZoneKey(zoneX, zoneY));
    }

    public hasConnection(zoneX: number, zoneY: number, direction: Direction): boolean {
        const connections = this.getConnections(zoneX, zoneY);
        return !!(connections && connections[direction] !== null);
    }

    public getExitPosition(zoneX: number, zoneY: number, direction: Direction): number | null {
        const connections = this.getConnections(zoneX, zoneY);
        return connections ? connections[direction] : null;
    }
