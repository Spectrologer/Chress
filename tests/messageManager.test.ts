import { MessageManager } from '../src/ui/MessageManager';
import { getVoiceSettingsForName } from '../src/ui/VoiceSettings';
...
describe('MessageManager typing & blip behavior', () => {
  ...
  beforeEach(() => {
    ...
    global.requestAnimationFrame = (cb) => setTimeout(() => cb(performance.now()), 0) as any as number;
  ...
  test('TypewriterController detects character name from element', () => {
    const fakeGame = { messageLog: [], animationScheduler: makeFakeAnimationScheduler() } as any;
    ...
    const name = (mm.typewriterController as any)._detectCharacterNameForElement(container);
    expect(name).toBe('Penne');
  });

  test('showMessage with typewriterSpeed = 0 displays text immediately', async () => {
    const fakeGame = { messageLog: [], animationScheduler: makeFakeAnimationScheduler(), soundManager: { sfxEnabled: true } } as any;
    const mm = new MessageManager(fakeGame);
